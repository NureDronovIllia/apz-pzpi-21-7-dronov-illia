...
class ShiftService(BaseService):
    ...
    async def start_shift(self, shift_data: ShiftBase, current_user: User) -> ShiftData:
        ...
        await self.vehicle_repository.set_current_status(vehicle_id, VehicleStatuses.SHIFT)
    ...
    async def end_shift(self, shift_id: int, current_user: User) -> ShiftData:
        ...
        updated_shift = await self.shift_repository.update_shift(shift_id, (ShiftUpdate(end_time=datetime.utcnow())))
        await self.vehicle_repository.set_current_status(updated_shift.vehicle_id, VehicleStatuses.OFF_SHIFT)
